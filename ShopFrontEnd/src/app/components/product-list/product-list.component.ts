...
export class ProductListComponent implements OnInit {
  ...
  selectedSearchOption: string = 'discontinued';
  searchName: string = '';
    ...
    this.categoryService.getAllCategoryNames().subscribe({
      ...
      error: (error) => {
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Failed to load categories: ' + error.message
        });
      }
    });
  }
      ...
      .subscribe({
        next: (response) => {
          if (response.isSuccess && response.result) {
            ...
            this.errorMessage = response.errorMessage || 'Failed to load products';
          }
          this.loading = false;
        },
        error: (error) => {
          ...
          next: (response) => {
            if (response.isSuccess) {
              ...
              this.messageService.add({
                severity: 'error',
                summary: 'Error',
                detail: response.errorMessage || 'Failed to update product status'
              });
            }
          },
          error: (error) => {
            this.messageService.add({
              severity: 'error',
              summary: 'Error',
              detail: 'Failed to update product status: ' + error.message
            });
          }
        });
      }
